...
**核心原則**：
1. 評鑑知識來自 `lib/ai/evaluation-profiles/` — 寫文章前必須讀取對應機構的 profile，**絕對不自行編造評鑑條文**。
2. 產品功能描述（「報告汪如何/能/支援...」段落）來自 `docs/FEATURES.md` — 寫到報告汪實際能做什麼之前，**必須先讀此檔**，**絕對不編造產品功能**。詳見下方「產品功能描述準確性（必讀）」。
...
**為什麼這條獨立成段**：2026-05-15 曾發生編造產品功能事故——`disability-welfare-management-system-comparison-2026` 文章宣稱「ISP 三部曲快速起稿（AI 自動生成 ISP）」「多元障別適配（智能/自閉症/肢體/多重障礙各有 ISP 框架）」，皆為憑印象編造，codebase 完全無此實作，事後需 fix script 補救（commit e0ed445）。

### 強制流程

當文章中出現任何「報告汪可以做 X」「報告汪的 AI 會幫你 X」「報告汪支援 X」這類描述時：

1. **先讀 `docs/FEATURES.md`**（產品功能 SSOT，2026-05-15 建立）
2. 確認該功能存在於 A–F 任一節中，並引用其用詞
3. 若功能不存在於 SSOT → **不准寫**；若認為應該存在 → 先 grep codebase 驗證，找到證據才寫，並回頭更新 SSOT

### 黑名單詞彙（檢測到任一就要重寫）

| ❌ 禁用 | 原因 | ✅ 替換 |
|---|---|---|
| 「AI 自動生成 ISP / 評估報告 / 計畫書」 | 報告汪 AI **改既有文件**，不從零生成 | 「上傳既有文件，AI 比對評鑑指標分析缺漏」 |
| 「AI 起稿 / 快速起稿 / 從零撰寫」 | 同上 | 「AI 改寫既有段落」「AI 結構化為 SOAP」 |
| 「智能障別適配」「障別自動分流」 | codebase 無障別分流邏輯 | 「依機構類型套用對應評鑑 profile」 |
| 「即時協同編輯」「多人同時編輯」 | 無 Y.js/CRDT，是「以標籤分享存取」 | 「以標籤為單位分享給多位使用者共同編輯（非即時）」 |
| 「AI 自動填表 / 自動帶入欄位」 | 無此功能 | 「AI 分析後產生建議清單，由使用者人工補強」 |
| 「自動分類評鑑條項」「自動標注佐證」 | 無 auto-tag 實作 | 「使用者標注標籤後，AI 依評鑑指標分析」 |
| 「品質指標自動監測 / 閾值警示 / PBS 圖表」 | 機構管理系統廠商功能，非報告汪 | 不寫此功能，或明示「報告汪不含此功能，建議結合外部系統」 |
| 「Pro 方案 / 進階訂閱 / 無限額度」 | 無付費實作，全部 free tier 1 次/天 | 「免費使用，每日 1 次 AI 配額」 |
| 「報告段落 AI 即時 streaming」 | `/api/reports/[id]/ai` 是 JSON 回應 | 「AI 段落改寫」（不提 streaming）；A2/A3 evaluation 才是 streaming |

### 真實 AI 功能對照（必背）

報告汪三個 AI 路由實際做的事（2026-05-15 snapshot，以 `docs/FEATURES.md` 為準）：

1. `/api/reports/[id]/ai`：段落改寫 + SOAP 結構化（**修改既有段落**，不消耗每日配額）
2. `/api/reports/evaluation`：跨報告五面向評鑑分析（**對既有報告評分**，streaming，消耗配額）
3. `/api/documents/[id]/ai`：4 個 action — analyze / improve / summarize / extract-data（**對既有文件分析**，streaming，消耗配額）

**沒有任何「從零生成」「自動填表」「智能適配」API**。
...
**文章骨架**：
```html
<!-- 注意：文章 body 不放 重點摘要 blockquote。摘要由 excerpt 欄位承擔（自動渲染為 TL;DR 卡）。
     body 第一段直接以情境式 <p> 切入，呼應 excerpt 但避免重複資訊。 -->

<p>[情境 hook：用「你有沒有這種經驗...」「評鑑通知一到，你最怕的是...」等問句帶讀者進入痛點]</p>
...
**文章骨架**：
```html
<!-- 摘要由 excerpt 欄位承擔；body 直接進入第一段情境鋪陳 -->

<p>[情境 hook：第 N 條最容易在哪個環節被扣分？把讀者帶進實務場景]</p>
...
<p>[情境 hook：兩者差異常造成什麼實務困擾？讀者選錯會付出什麼代價？]</p>
...
**文章骨架**：
```html
<!-- 摘要由 excerpt 欄位承擔；body 直接進入第一個分類 H2 -->
...
<p>[情境 hook：新制對機構日常作業帶來什麼直接改變？哪一項影響最大？]</p>
...
**文章骨架**：
```html
<!-- 摘要由 excerpt 欄位承擔；body 直接進入「使用說明」 H2 -->
...
### 4. 開頭摘要策略（TL;DR + 情境 hook）

摘要分兩層，各自獨立、不重複：

**層一：`excerpt` 欄位（≤150 字）→ 自動渲染為 TL;DR 卡片**
- 由頁面元件 `<BlogTldr>` 在文章標題下方、目錄上方顯示
- 給「30 秒掃讀者」和「AI Answer Engine」：直接說明本文提供什麼、解決什麼問題
- 也用於列表卡片、SEO meta description、OG 標籤

**層二：body 第一段 `<p>` → 情境 hook**
- 給「準備認真閱讀的人」：製造痛點共鳴，帶讀者進入主題
- 範例：「你有沒有這種經驗：評鑑通知一到...」、「拿到基準手冊那一刻...」
- 30–60 字即可，不需重複 excerpt 的資料

⚠️ **禁止**在 body 開頭再放 `<blockquote>重點摘要：...</blockquote>`——TL;DR 卡已承擔此角色，重複會造成視覺冗餘。

AI 系統抓取文章時，TL;DR 卡（excerpt）是第一個結構化區塊，確保被正確引用。
...
| `excerpt` | ≤150 字 | **必填**。渲染為文章頭部 TL;DR 卡（`<BlogTldr>`），同時用於 SEO meta description 與列表卡片。應直接說明本文解決什麼問題、提供什麼具體價值。 | 評鑑通知一到，你是不是也盯著 45 題不知從哪開始？本文幫你把 115 年度日照評鑑五大面向拆解清楚... |
...
**GEO**
- [ ] `excerpt` 欄位已填寫（≤150 字，含主關鍵字）→ 自動渲染為 TL;DR 卡
- [ ] Body 第一段為情境 hook（不重複 excerpt 內容）
...
**產品功能準確性**（提及「報告汪如何/能/支援...」時必檢）
- [ ] 已讀取 `docs/FEATURES.md` 並引用其用詞
- [ ] 沒有黑名單詞彙：「AI 自動生成」「快速起稿」「障別適配」「即時協同」「自動填表」「自動分類條項」「Pro/進階方案」
- [ ] 每個產品功能描述都能對應到 `docs/FEATURES.md` 的 A–F 任一條目
- [ ] 動詞用「分析／比對／改寫／建議／結構化」而非「生成／起稿／自動產生／智能適配」
