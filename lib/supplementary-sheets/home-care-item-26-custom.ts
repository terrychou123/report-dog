...
function buildInfectionControlPlanSheet(): SheetData {
  ...
  push(["第八條\n（訓練規定）",
    "每年至少辦理一次感染管制教育訓練，新進人員職前訓練必修，訓練內容涵蓋：手部衛生、PPE 使用、個案感染症狀辨識、通報流程。訓練記錄存入教育訓練記錄表。",
    rowH(3)]);
  cs[`${r - 1}_0`] = { ht: 0, vt: 0, bold: true, tb: 2 };
  cs[`${r - 1}_1`] = { ht: 0, vt: 0, tb: 2 };
  ...
  push(["第九條\n（稽核頻率）",
    "督導員每季應抽查至少五名照服員之感染管制執行情形（含手部衛生、PPE 使用及器材清潔），結果記錄於「入戶手部衛生防護稽核表」，並於年度感染管制報告中彙整說明。",
    rowH(3)]);
  cs[`${r - 1}_0`] = { ht: 0, vt: 0, bold: true, tb: 2 };
  cs[`${r - 1}_1`] = { ht: 0, vt: 0, tb: 2 };
  ...
  push(["第十條\n（修訂）",
    "本計畫每年至少修訂一次，依衛生主管機關最新指引更新，由負責人核定後實施。",
    rowH(2)]);
  cs[`${r - 1}_0`] = { ht: 0, vt: 0, bold: true, tb: 2 };
  cs[`${r - 1}_1`] = { ht: 0, vt: 0, tb: 2 };
