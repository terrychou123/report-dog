...
function buildFallSopSheet(): SheetData {
  ...
  push(["訓練規定",
    "本 SOP 應列為職前訓練必修課程，並每年辦理一次實地演練，演練紀錄保存於訓練記錄表。",
    rowH(2)]);
  cs[`${r - 1}_0`] = { ht: 0, vt: 0, bold: true };
  cs[`${r - 1}_1`] = { ht: 0, vt: 0, tb: 2 };
...
function buildAcuteIllnessSopSheet(): SheetData {
  ...
  push(["訓練規定",
    "照顧服務員應完成急救訓練（含 CPR+AED 操作），建議每二年更新訓練認證。本 SOP 列為職前訓練必修。",
    rowH(2)]);
  cs[`${r - 1}_0`] = { ht: 0, vt: 0, bold: true };
  cs[`${r - 1}_1`] = { ht: 0, vt: 0, tb: 2 };
...
function buildMissingSopSheet(): SheetData {
  ...
  push(["說明",
    "緊急聯絡窗口清單詳見「緊急聯絡窗口表」分頁，照服員應隨身攜帶或熟記，每半年由督導員更新。",
    rowH(2)]);
  cs[`${r - 1}_0`] = { ht: 0, vt: 0, bold: true };
  cs[`${r - 1}_1`] = { ht: 0, vt: 0, tb: 2 };
