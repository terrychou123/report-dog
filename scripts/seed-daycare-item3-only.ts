/**
 * 僅針對日照項目 3（服務契約簽訂情形）重新產生範本內容，
 * 其他項目完全不動。舊版內容會備份到 template_revisions（最多保留 5 筆）。
 *
 * Run:
 *   npx dotenv-cli -e .env.local -- tsx scripts/seed-daycare-item3-only.ts
 */
...
const TARGET_ITEM_ID = 3;
const MAX_REVISIONS = 5;
const SEED_SCRIPT_USER_ID = "seed-script-item3";
...
async function main() {
  ...
  try {
    ...
    // 2. 產生新內容（archetype 分頁 + 6 張客製契約分頁）
    ...
    if (existingList.length > 1) {
      console.error(
        `❌ DB 中有 ${existingList.length} 筆重複範本：${title}，停止執行`,
      );
      process.exit(1);
    }
    ...
    console.log(`   範本分頁數：${sheetCount}（含查核表 + 6 個契約範本分頁）`);
