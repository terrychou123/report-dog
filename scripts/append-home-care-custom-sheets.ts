 ...
 *
 * Run (dry-run):
 *   npx dotenv-cli -e .env.local -- tsx scripts/append-home-care-custom-sheets.ts
 *
 * Run (write to DB):
 *   npx dotenv-cli -e .env.local -- tsx scripts/append-home-care-custom-sheets.ts --apply
 *
 * Run specific items only:
 *   npx dotenv-cli -e .env.local -- tsx scripts/append-home-care-custom-sheets.ts --apply --ids 2,4,10
 */
...
const FACILITY_TYPE = "home-care";
const MAX_REVISIONS = 5;
const SEED_SCRIPT_USER_ID = "seed-script-home-care-custom-sheets";
...
async function main() {
  ...
  const allItems = homeCareProfile.sections.flatMap((s) => s.items);
  ...
  try {
    for (const itemId of targetItemIds) {
      ...
      if (!item) {
        console.warn(`⚠️  找不到 home-care 項目 ${itemId} 的 profile 資料，跳過`);
        warningCount++;
        continue;
      }
