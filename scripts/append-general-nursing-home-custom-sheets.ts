 ...
 *
 * Run (dry-run):
 *   npx dotenv-cli -e .env.local -- tsx scripts/append-general-nursing-home-custom-sheets.ts
 *
 * Run (write to DB):
 *   npx dotenv-cli -e .env.local -- tsx scripts/append-general-nursing-home-custom-sheets.ts --apply
 *
 * Run specific items only:
 *   npx dotenv-cli -e .env.local -- tsx scripts/append-general-nursing-home-custom-sheets.ts --apply --ids 2
 *   npx dotenv-cli -e .env.local -- tsx scripts/append-general-nursing-home-custom-sheets.ts --apply --ids 3,4,5,6,7,8,9,11,12
 */
...
const FACILITY_TYPE = "general-nursing-home";
const MAX_REVISIONS = 5;
const SEED_SCRIPT_USER_ID = "seed-script-general-nursing-home-custom-sheets";
...
async function main() {
  ...
  const allItems = generalNursingHomeProfile.sections.flatMap((s) => s.items);
  ...
  try {
    for (const itemId of targetItemIds) {
      ...
      if (!item) {
        console.warn(`⚠️  找不到 general-nursing-home 項目 ${itemId} 的 profile 資料，跳過`);
        warningCount++;
        continue;
      }
