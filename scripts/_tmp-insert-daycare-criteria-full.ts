#!/usr/bin/env tsx
// 一次性插入腳本：daycare-eval-115-criteria-full-2026
// 用法：npx tsx --env-file=.env.local scripts/_tmp-insert-daycare-criteria-full.ts
// 插入後 status=draft，確認內容後再改為 published
...
const EXCERPT =
  "115 年度臺北市日照中心評鑑共 43 項正式基準＋2 項加分題，分壹個案權益、貳專業照護、參經營管理、肆安全環境四大章節。本文依官方評鑑基準逐條列出審查重點與查核方式，是評鑑準備期間最完整的條文速查參考。";
...
async function main() {
  ...
  if (existing) {
    console.log(`⚠️  文章已存在（status: ${existing.status}）：${SLUG}`);
    console.log("   不重複插入。如需更新請手動編輯。");
    process.exit(0);
  }

  await db.insert(blogPosts).values({
    slug: SLUG,
    title: "115 年度日照中心評鑑基準完全解析：43 項逐條速查與查核方式",
    excerpt: EXCERPT,
    content: CONTENT,
    category: "日間照顧",
    tags: ["日照評鑑", "115年度", "評鑑基準", "日間照顧", "備審文件"],
    seoTitle: "115年日照中心評鑑基準完全解析｜43項逐條速查｜報告汪",
    seoDescription:
      "115年度臺北市日照中心評鑑共43項正式基準，涵蓋個案權益、專業照護、經營管理、安全環境四大章節。本文依官方評鑑基準逐條列出審查重點與委員查核方式，附完整角色分工表與五大 FAQ，是日照評鑑準備最完整的條文速查參考。",
    coverImageUrl: null,
    status: "draft",
    publishedAt: null,
  });

  console.log(`✅ 已插入草稿：${SLUG}`);
  console.log(`   請前往 https://reportwang.com/blog-admin 確認內容後設為 published`);
  console.log(`   或執行：npx tsx --env-file=.env.local scripts/_tmp-publish-daycare-criteria-full.ts`);
}
