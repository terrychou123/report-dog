/**
 * 僅針對日間照顧項目 45（機構權益保障・監視錄影設備）重新產生範本內容，
 * 其他項目完全不動。舊版內容會備份到 template_revisions（最多保留 5 筆）。
 *
 * Run:
 *   npx dotenv-cli -e .env.local -- tsx scripts/seed-daycare-item45-only.ts
 */
...
const TARGET_ITEM_ID = 45;
const MAX_REVISIONS = 5;
const SEED_SCRIPT_USER_ID = "seed-script-item45";
...
async function main() {
  ...
  try {
    ...
    // 2. 產生新內容（含自訂 6 分頁）
    const supplementaryDefs = getSupplementaryDefs(
      daycareProfile.id,
      item.id,
    );
