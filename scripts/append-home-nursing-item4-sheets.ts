/**
 * 將居家護理所「4 A4 個案緊急或意外事件處理」的 1 個自訂補充分頁，
 * Append 到 UUID 6be3719a-fc89-4149-aec9-2aa52b4890cb 範本末端。
 *
 * - 冪等：分頁名已存在時跳過，可重複執行。
 ...
 *
 * Run (dry-run):
 *   npx dotenv-cli -e .env.local -- tsx scripts/append-home-nursing-item4-sheets.ts
 *
 * Run (write to DB):
 *   npx dotenv-cli -e .env.local -- tsx scripts/append-home-nursing-item4-sheets.ts --apply
 */
...
import { buildHomeNursingItem4CustomSheets } from "../lib/supplementary-sheets/home-nursing-item-4-custom";
import type { SheetData } from "../lib/excel-template-builder";

const TEMPLATE_ID = "6be3719a-fc89-4149-aec9-2aa52b4890cb";
const MAX_REVISIONS = 5;
const SEED_SCRIPT_USER_ID = "seed-script-home-nursing-item4-custom-sheets";
...
async function main() {
  ...
  try {
    ...
    const newSheets = buildHomeNursingItem4CustomSheets();
